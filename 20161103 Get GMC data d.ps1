...
Notes
  ...
  Advantage of IE (compared to using other agents like powershell itself or the .NET client) is that the site will always recognise and accept it as a browser wheras other clients are regularly denied access (on parts of NHS related websites, e.g. HSCIC)
  Main Disadvantage, just slow and memory hogging

Output:
  Delimited file with delimiter "¬"
  The qualifications column is delimited with '<br>' (wh the quotes) if there is more than one qualification
  The History column in turn is delimited with '|' (wh the quotes), it has somewhere between 3 and 188 delimiters
  History is a table with 3 columns, except when the next Column has 'View' in it, in which case it becomes the 4th column of the previous row
  History can have 3 parts:
    Fitness to practise history since 20 October 2005
    with headers: From|To|Status
    Registration and licensing history since 20 October 2005
    with headers: From|To|Status
    Hearings 
    with headers: Date|Hearing
  If the History column only contains the word 'Details' and has a link to a document that link is grabbed instead of the word 'Details'
  
  Probably should output in JSON, though that would increase filesizes.
...
# change window title (optional)
$host.ui.RawUI.WindowTitle = "Extract GMC numbers d"                            <# CHANGE #>
# for restart; number to cycle forward to; should be first number in file on first run
$gmctocheck1 = "0014922"                                                        <# CHANGE #>
...
$inputfile = "S:\_ue\GMCnumbers_d.csv"                                          <# CHANGE #>
# output files
$outputfile = "S:\_ue\GMCdata_d.csv"                                            <# CHANGE #>
$erroroutputfile = "S:\_ue\errorGMCdata_d.csv"                                  <# CHANGE #>

# header in output (commented out so restarts can use same file(s))
...
#        $ie.document.title
  ...
  #          $ie.document.title
            ...
            For ($i=0; $i -lt $numcells; $i++){
              ...
              if($datah -eq "Details"){
                #grab link from HTML with regex
                $datah1 = $null
                $datah1 = $listapplettablerows[$i].innerHTML -replace "<a.+href=`"(.+)`" target.+</a>",'$1'
                # check if link isn't a document
                if ($datah1.Substring($datah1.Length-3,3) -eq "htm"){
                  # not a document do nothing
                }else{
                  # assign link
                  $datah = $datah1
                }
